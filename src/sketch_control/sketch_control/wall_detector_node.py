...
알고리즘:
  PointCloud2 → numpy → open3d.PointCloud → segment_plane(RANSAC) → (a,b,c,d) + inliers
...
class WallDetectorNode(Node):
    ...
    def _on_cloud(self, msg: PointCloud2):
        ...
        # open3d PointCloud + voxel downsample (RANSAC 속도용)
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(points.astype(np.float64))
        pcd = pcd.voxel_down_sample(VOXEL_SIZE)
        self.get_logger().info(f"[debug] points.shape={points.shape}, pcd.points 수={len(pcd.points)}")
        ...
        distances = np.linalg.norm(points_arr, axis=1)  # zed_left_camera_frame 기준 거리
